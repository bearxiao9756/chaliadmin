...
<el-date-picker class="!w-380px" v-model="searchInfo.updatedAtRange" type="datetimerange" range-separator="至"  start-placeholder="开始时间" end-placeholder="结束时间"></el-date-picker></el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="命令" prop="command" width="120" />
   ...
   <template #default="scope">{{ formatDate(scope.row.updatedAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateTeamChaliBotCommandsFunc(scope.row)">编辑</el-button>
...
import {
  ...
  deleteTeamChaliBotCommands,
  deleteTeamChaliBotCommandsByIds,
  updateTeamChaliBotCommands,
  findTeamChaliBotCommands,
  getTeamChaliBotCommandsList
...
const getTableData = async() => {
  const table = await getTeamChaliBotCommandsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteTeamChaliBotCommandsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteTeamChaliBotCommandsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTeamChaliBotCommandsFunc = async(row) => {
    const res = await findTeamChaliBotCommands({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteTeamChaliBotCommandsFunc = async (row) => {
    const res = await deleteTeamChaliBotCommands({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
